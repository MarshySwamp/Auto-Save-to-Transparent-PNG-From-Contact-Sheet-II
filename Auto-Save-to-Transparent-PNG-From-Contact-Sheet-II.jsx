...
psxui.createFontPanel = function (pnl, ini, label, lwidth) {
    ...
    pnl.family = pnl.add('dropdownlist', [x, y, x + 200, y + 22]);
    pnl.family.helpTip = ZStrs.FontTip;
    x += 210;
    pnl.style = pnl.add('dropdownlist', [x, y, x + 170, y + 22]);
    pnl.style.helpTip = ZStrs.FontStyleTip;
    x += 180;
    pnl.fontSize = pnl.add('editnumber', [x, y, x + 30, y + 22], 12, ContactSheetUI.MIN_FONTSIZE, ContactSheetUI.MAX_FONTSIZE, { numberFormat: "integer" });
    ...
    //we have used editnumber control for fontSize, so blocking of invalid input will be done internally
    ...
    pnl.getFont = function () {
        ...
        return { font: font.postScriptName, size: pnl.fontSize.value };
...
ZStrings.SortByFilename =
    localize("$$$/JavaScripts/ContactSheet2/SortByFilnemame=Sort By Name");
...
ZStrings.FileSelectedFmt =
    localize("$$$/JavaScripts/ContactSheet2/FileSelectedFmt=%%d file selected");
...
ContactSheetUI.prototype.getPageAndImageCounts = function () {
    ...
    opts.rowCount = appPnl.layoutPanel.rows.value;
    opts.columnCount = appPnl.layoutPanel.cols.value;
    if (opts.folder) {
...
// Description: convert a value from one unit to another unit
// Input: value - the value which has to be converted
//        fromUnit - the current unit of the value
//        toUnit - the result unit
//        baseUnit - the resolution base
// Return: value converted to toUnit
//
ContactSheetUI.convertValueToUnit = function (value, fromUnit, toUnit, baseUnit) {
    var fromValue = UnitValue(value, fromUnit);
    if (!baseUnit) {
        baseUnit = UnitValue(1 / 72, "in");
    }
    fromValue.baseUnit = baseUnit;
    var result = fromValue.as(toUnit);
    return result;
};
...
// UI constants
//
ContactSheetUI.CSII_WIDTH = 600;
ContactSheetUI.CSII_HEIGHT = 665;
...
ContactSheetUI.prototype.createPanel = function (pnl, ini) {
    ...
    pnl.thumbnailPanel = pnl.add('panel', [xx, yy, xx + panelWidth + 135, yy + 135],
        ZStrings.Thumbnails,
        { name: 'thumbnailPanel' });
    ...
    pnl.captionPanel = pnl.add('panel', [xx, yy, xx + panelWidth + 135, yy + 50],
        ZStrings.FilenameAsCaption,
        { name: 'captionPanel' });
    ...
    pnl.ctrlPanel = pnl.add('group',
        [xx + panelWidth + 5, 0, pnl.bounds.width - 5, 400],
        { name: 'ctrlPanel' });
...
ContactSheetUI.prototype.createCSIISourcePanel = function (pnl, opts) {
    ...
    var menuWidth = Math.max(maxW + 35, width);
    ...
    ZStrings.AddOpenFiles, ZStrings.AddBridgeFiles, ZStrings.SortByFilename];
    ...
    pnl.fileList = pnl.add('listbox', [x, y, x + fileListWidth, y + 197], undefined,
    ...
    y += btnH + 5;
    pnl.sortBtn = pnl.add('button', [x, y, x + btnW, y + btnH],
        ZStrings.SortByFilename);

    pnl.fileWidgetList = [pnl.fileList, pnl.browseBtn, pnl.removeBtn,
    pnl.addOpenBtn, pnl.addBridgeBtn, pnl.sortBtn];
    ...
    // Subfolders
    pnl.recurse = pnl.add('checkbox', [x, y, pnl.bounds.width - x, y + 25],
        ZStrings.IncludeSubfolders,
    ...
    // Group
    pnl.group = pnl.add('checkbox', [x, y, pnl.bounds.width - x, y + 25],
        ZStrings.GroupByFolder,
    ...
    pnl.imageSource.onChange = function () {
        ...
        if (sel == ZStrings.Bridge) {
            var files = ContactSheetUI.getImageFilesFromBridge();
            if (files.length == 1) {
                var txt = ZStrings.FileSelectedFmt.sprintf(files.length);
            } else {
        ...
        if (sel == ZStrings.CurrentDocs) {
            if (app.documents.length == 1) {
                var txt = ZStrings.FileSelectedFmt.sprintf(app.documents.length);
            } else {
...
ContactSheetUI.prototype.updateUnitWidget = function (obj, type, base) {
    ...
    if (obj.type == "editnumber")
        obj.value = _approximate(val);
    else
...
ContactSheetUI.prototype.createCSIIDocumentPanel = function (pnl, opts) {
    ...
    var labelsLeft = [ZStrings.UnitsLabel, ZStrings.WidthLabel, ZStrings.HeightLabel,
    ZStrings.ResolutionLabel, ZStrings.ColorProfileLabel];

    var labelsRight = [ZStrings.ModeLabel, ZStrings.BitDepthLabel];

    var maxWLeft = _findMaxWidth(pnl, labelsLeft);
    var maxWRight = _findMaxWidth(pnl, labelsRight);

    var x = gutter;
    var gutterL = Math.max(maxWLeft + 5, gutter);
    var gutterR = Math.max(maxWRight + 5, gutter);
    ...
    pnl.unitsLabel = pnl.add('statictext',
        [xx, yy + tOfs, xx + gutterL, yy + 22 + tOfs],
        ZStrings.UnitsLabel);
    ...
    pnl.units.onChange = function () {
        try {
            ...
            // change the min and max value of pnl.width
            pnl.width.minvalue = ContactSheetUI.convertValueToUnit(ContactSheetUI.MIN_PIXELS, "px", type, pnl._baseUnit);
            pnl.width.maxvalue = ContactSheetUI.convertValueToUnit(ContactSheetUI.MAX_PIXELS, "px", type, pnl._baseUnit);
            processor.updateUnitWidget(pnl.width, type, pnl._baseUnit);
            // change the min and max value of pnl.height
            pnl.height.minvalue = ContactSheetUI.convertValueToUnit(ContactSheetUI.MIN_PIXELS, "px", type, pnl._baseUnit);
            pnl.height.maxvalue = ContactSheetUI.convertValueToUnit(ContactSheetUI.MAX_PIXELS, "px", type, pnl._baseUnit);
    ...
    xx += gutterL;
    pnl.width = pnl.add('editnumber', [xx, yy, xx + textW, yy + 20],
        opts.docWidth || CSIIDefaults.width,
        ContactSheetUI.convertValueToUnit(ContactSheetUI.MIN_PIXELS, "px", unitType, pnl._baseUnit),
        ContactSheetUI.convertValueToUnit(ContactSheetUI.MAX_PIXELS, "px", unitType, pnl._baseUnit),
        { name: 'width', decimalDigits: 3 });
    pnl.width._last = pnl.toPixels(pnl.width.value, unitType);
    pnl.width.helpTip = ZStrings.WidthTip;

    pnl.width.onChange = function () {
        var pnl = this.parent;
        var type = delocalizeUnitType(pnl.units.selection.text);
        var n = pnl.width.value;
    ...
    // we have used editnumber control for pnl.width, so blocking of invalid input will be done internally
    ...
    xx += gutterL;
    pnl.height = pnl.add('editnumber', [xx, yy, xx + textW, yy + 20],
        opts.docHeight || CSIIDefaults.height,
        ContactSheetUI.convertValueToUnit(ContactSheetUI.MIN_PIXELS, "px", unitType, pnl._baseUnit),
        ContactSheetUI.convertValueToUnit(ContactSheetUI.MAX_PIXELS, "px", unitType, pnl._baseUnit),
        { name: 'height', decimalDigits: 3 });
    pnl.height.helpTip = ZStrings.HeightTip;
    pnl.height._last = pnl.toPixels(pnl.height.value, unitType);

    pnl.height.onChange = function () {
        var pnl = this.parent;
        var type = delocalizeUnitType(pnl.units.selection.text);

        var n = pnl.height.value;
    ...
    // we have used editnumber control for pnl.height, so blocking of invalid input will be done internally
    ...
    pnl.resolution = pnl.add('editnumber', [xx, yy, xx + textW, yy + 20],
        opts.resolution || CSIIDefaults.resolution,
        undefined,
        undefined,
        { name: 'resolution', decimalDigits: 3 });
    ...
    ContactSheetUI.addPositiveNumberFilter(pnl.height);
    // we have used editnumber control for pnl.resolution, so blocking of invalid input will be done internally
    //ContactSheetUI.addPositiveNumberFilter(pnl.resolution);
    ...
    // set min and max value for resolution
    var resType = ((pnl.resUnits.selection.text == ZStrings.PixelsPerInch) ?
        ZStrs.Units_IN : ZStrs.Units_CM);

    if (resType == ZStrs.Units_IN) {
        pnl.resolution.minvalue = ContactSheetUI.MIN_RES_INCHES;
        pnl.resolution.maxvalue = ContactSheetUI.MAX_RES_INCHES;
    } else {
        pnl.resolution.minvalue = ContactSheetUI.MIN_RES_CM;
        pnl.resolution.maxvalue = ContactSheetUI.MAX_RES_CM;
    }
    ...
    pnl.resUnits.onChange = function () {
        ...
        if (resType == ZStrs.UnitsCM) {
            pnl.resolution.minvalue = ContactSheetUI.MIN_RES_CM;
            pnl.resolution.maxvalue = ContactSheetUI.MAX_RES_CM;
            pnl.resolution.value = _approximate(res / 2.54);
        } else {
            pnl.resolution.minvalue = ContactSheetUI.MIN_RES_INCHES;
            pnl.resolution.maxvalue = ContactSheetUI.MAX_RES_INCHES;
            pnl.resolution.value = _approximate(pnl.resolution._inches);
        }
    ...
    pnl.resolution._value = pnl.resolution.value;
    pnl.resolution._inches = _resAsInches(pnl.resolution.value, type);

    pnl.resolution.onChange(true);

    var yy_saved = yy + lineH;
    //xOfs = gutterL + menuW + 5 + xOfs + 50;
    xOfs = Math.max(300, (gutterL + menuW + 5 + xOfs + 50));
    ...
    // Flatten All Layers
    //
    pnl.flattenAllLayers = pnl.add('checkbox', [xx, yy, pnl.bounds.width - 10, yy + 22],
        ZStrings.FlattenAllLayers,
    ...
    pnl.modeLabel = pnl.add('statictext',
        [xx, yy + tOfs, xx + gutterR, yy + 22 + tOfs],
        ZStrings.ModeLabel);
    ...
    pnl.colorProfile = pnl.add('dropdownlist', [xx, yy, xx + menuW + 35, yy + 22],
        pnl.RGB_PROFILES,
        { name: 'colorProfile' });
...
ContactSheetUI.prototype.createCSIIThumbnailPanel = function (pnl, opts) {
    ...
    xx += gutter;
    pnl.cols = pnl.add('editnumber', [xx, yy, xx + 50, yy + 20],
        opts.columnCount || CSIIDefaults.cols,
        ContactSheetUI.MIN_COLS_ROWS,
    ...
    // we have used editnumber control for pnl.cols, so blocking of invalid input will be done internally
    ...
    xx += gutter;
    pnl.rows = pnl.add('editnumber', [xx, yy, xx + 50, yy + 20],
    ...
    ContactSheetUI.addNumericFilter(pnl.cols);
    // we have used editnumber control for pnl.cols, so blocking of invalid input will be done internally
    //ContactSheetUI.addNumericFilter(pnl.rows);
    ...
    // Best Fit
    //
    pnl.bestFit = pnl.add('checkbox', [xx, yy, pnl.bounds.width - xx, yy + 22],
        ZStrings.RotateForBestFit,
        { name: 'bestFit' });
    pnl.bestFit.helpTip = ZStrings.RotateForBestFitTip;
    pnl.bestFit.value = CSIIDefaults.bestFit;

    //xOfs = gutter + menuW + 5 + xOfs + 50;
    xOfs = 300;
    xx = xOfs;
    yy = yOfs;
    gutter = 90;
    ...
    // Autospacing
    //
    pnl.autoSpacing = pnl.add('checkbox', [xx, yy, pnl.bounds.width - xx, yy + 22],
        ZStrings.UseAutoSpacing,
        { name: 'autoSpacing' });
...
ContactSheetUI.prototype.createCSIICaptionPanel = function (pnl, opts) {
    ...
    pnl.captionEnabled = pnl.add('checkbox', [xx, yy - 5, xx + 26, yy + 30], '',
        { name: 'captionEnabled' });
...
ContactSheetUI.prototype.createCSIIControlPanel = function (pnl, opts) {
    ...
    xx += 5;
    yy += (isWindows() ? 40 : 35);
    textHeight = (isWindows() ? 67 : 87);

    pnl.esc = pnl.add('statictext', [xx, yy, xx + btnW - 10, yy + textHeight], ZStrings.PressESC,
        { multiline: true });
...
ContactSheetUI.prototype.validateCSII = function () {
    ...
    try {
        ...
        var cols = self.thumbnailPnl.cols.value;
        var rows = self.thumbnailPnl.rows.value;
...
ContactSheetUI.prototype.readDocumentSettings = function () {
    ...
    settings.@width = pnl.width.value;
    settings.@height = pnl.height.value;
    settings.@resolution = pnl.resolution.value;
...
ContactSheetUI.prototype.writeDocumentSettings = function (settings) {
    ...
    pnl.width.value = psx.getXMLValue(settings.@width, CSIIDefaults.width);
    pnl.height.value = psx.getXMLValue(settings.@height, CSIIDefaults.height);
    ...
    pnl.resolution.value = psx.getXMLValue(settings.@resolution,
...
ContactSheetUI.prototype.readThumbnailSettings = function () {
    ...
    settings.@cols = pnl.cols.value;
    settings.@rows = pnl.rows.value;
...
ContactSheetUI.prototype.writeThumbnailSettings = function (settings) {
    ...
    pnl.cols.value = psx.getXMLValue(settings.@cols, CSIIDefaults.cols);
    pnl.rows.value = psx.getXMLValue(settings.@rows, CSIIDefaults.rows);
...
if (app.documents.length > 0) {
    ...
    (function () {
        ...
        function saveOpenDocs() {
            ...
            // Select and delete the Background layer if it exists
            try {
            ...
            var saveFilePNG = new File(new File(outputFolder + '/' + docName + '.png'));
            savePNG(saveFilePNG);
            ...
            function savePNG(saveFilePNG) {
                ...
                app.activeDocument.saveAs(saveFilePNG, pngOptions, true, Extension.LOWERCASE);
            }
        }
...
// must be the last thing, if user hits cancel on the dialog then no action/event gets recorded
gScriptResult;
